...
# ────────────────────────────── CONFIG ──────────────────────────────
PROMPT_TXT = "ddpo_pytorch/assets/hps_v2_all_eval.txt"
with open(PROMPT_TXT, encoding="utf-8") as f:
    PROMPTS = [ln.strip() for ln in f if ln.strip()]
PROMPTS.sort(key=len, reverse=True)  # 긴 프롬프트부터 매칭하도록 정렬
...
def identify_prompt(fname: str) -> str:
    ...
    # 1. 파일명에서 숫자와 특수문자 제거
    clean_name = re.sub(r'[0-9_\-.]', ' ', base).strip()
    
    # 2. 정확한 프롬프트 매칭 시도
    for prompt in PROMPTS:
        if prompt.lower() in clean_name:
            return prompt
            
    # 3. 가장 많은 단어가 매칭되는 프롬프트 찾기
    max_matches = 0
    best_prompt = None
    ...
    for prompt in PROMPTS:
        ...
        matches = len(prompt_words & name_words)
        
        if matches > max_matches:
            max_matches = matches
            best_prompt = prompt
    
    if best_prompt and max_matches >= 2:  # 최소 2개 단어는 매칭되어야 함
        return best_prompt
        
    raise ValueError(f"prompt not found in filename: {fname}")

def parse_pickscore(fname: str):
    """파일 이름에서 PickScore를 추출합니다."""
    m = re.search(r"_pick([-+]?[0-9]*\.?[0-9]+)\.jpg$", fname, re.I)
...
def evaluate_folder(folder: str, K: int = 20) -> Dict[str, float]:
    ...
    # 2) 품질 점수 / 임베딩 저장
    # -----------------------------------------------------------------
    qual = {k: [] for k in ["pickscore", "hps", "imagereward", "clip"]}  # pick 제거
    ...
    for path in imgs:
        ...
        # ---- pickscore (파일명에 점수 포함된 경우) ----
        pick_score = parse_pickscore(path)
        if pick_score is not None:
            qual["pickscore"].append(pick_score)
...
def collect_folders(base_dir: str, every: int):
    ...
    for name in os.listdir(base_dir):
        ...
        if m:
            epoch = int(m.group(1))
            if epoch % every == 0:       # epoch 1 필수 조건 제거
                eval_f.append(name)
...
def run(base_dir: str, every: int):
    ...
    # plotting spec
    metrics = [
        ("mean_pickscore","std_pickscore","PickScore (filename)","pickscore"),
    ...
    for key,std_key,label,suf in metrics:
        ...
        # eval 데이터가 있는 경우에만 plot
        if any(not np.isnan(x) for x in mean_eval):
        ...
        # search 데이터가 있는 경우에만 plot
        if any(not np.isnan(x) for x in mean_search):
        ...
        plt.title(f"{label}: eval vs search"); plt.grid(alpha=.3)
        if plt.gca().get_legend_handles_labels()[0]:  # legend가 있는 경우에만 표시
            plt.legend()
        plt.tight_layout()
